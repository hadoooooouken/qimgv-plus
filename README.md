...
* **Image Info Panel**: Displays EXIF/metadata for the current image. For PNG files generated by **ComfyUI**, automatically parses embedded KSampler workflow data and presents generation parameters (model, sampler, steps, CFG scale, seed, positive/negative prompts) in a structured view.
...
* **HDR Tone-Mapping**: Built-in tone-mapping pipeline for high-dynamic-range images, including 10-bit/12-bit AVIF HDR with Rec.2100 PQ or HLG transfer, OpenEXR, and Radiance HDR. Supports ITU-R BT.2408, Reinhard-Jodie, ACES Filmic, and Hable (Uncharted 2) operators with configurable target white levels.
...
* **Folder View Mode**: Seamless grid browsing with support for left mouse button rectangular selection and options to open only selected files (including folders). Features a format-filter dropdown to narrow the file list to a specific image format group (e.g. HEIF/HEIC, RAW, PNG).
...
The idea is to have an uncluttered, simple and easy to use UI. You can see UI elements only when you need them.
...
*Note: On Windows, you can use standard batch files (`.bat` / `.cmd`) directly. For PowerShell (`.ps1`) or Python (`.py`) scripts, you should configure them as a **command** (e.g. `powershell -File C:\script.ps1 %file%`).*
...
qimgv-plus provides multiple CPU and GPU scaling filters:
...
* **Formats**: Converts to **JPEG**, **PNG** (with 0-9 compression level slider; uses **libdeflate** for significantly faster export when `deflate.dll` is present), **WebP**, **JPEG-XL (JXL)**, **AVIF**, **BMP**, **TIFF**, and **QOI** with custom quality sliders.
...
* **Scaling Filters**: Supports standard filters (Nearest, Bilinear, MKS2021) and **Vulkan-accelerated AI Upscaling (Upscayl)** using customizable models such as `4xLSDIRCompactC3`.
...
* **Mapping**: Reads embedded source ICC profile and converts it to the display space. Images without an embedded profile are assumed to be standard `sRGB`.
...
* **Target White Level**: Adjustable reference white luminance tailored to your viewing environment:

  * **203 nits** — ITU-R BT.2408 default
  * **100 nits** — standard sRGB
  * **80 nits** — dim room
  * **300 nits** — bright room
