    ...
    if (cache.size() < qsizetype(size)) {
        ...
        while (num > 0) {
            ...
            if (c & 0x80) { // RLE pixels.
                assert(pixel_size <= 8);
                char pixel[8];
                qint64 start = dev->read(pixel, pixel_size);
                if (start < 0) {
                    memset(pixel, 0, pixel_size);
                    start = 0;
                }
                if (start < pixel_size) {
                    memset(&pixel[start], 0, pixel_size - start);
                }
                do {
                ...
                if (dataRead < (int)count) {
                    const int start = std::max(0, dataRead);
                    memset(&dst[start], 0, count - start);
                }
                dst += count;
            }
